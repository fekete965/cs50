...
from helpers import formatName, getTimeStamp, login_required, lookup, usd, validatePassword
...
def index():
    ...
    return render_template("portfolio.html", userSharesSummary=userSharesSummary, currentBalance=currentBalance)
...
def buy():
    ...
    if not float(shares).is_integer():
        flash("Shares must be a whole number")
        return render_template("buy.html"), 400
    parsedShares = int(shares)
    if parsedShares < 0:
        flash("Shares field cannot be negative")
        return render_template("buy.html"), 400
    ...
    totalValue = data.get("price", 0) * parsedShares
    ...
    flash(f"Congratulations!\nYou have bought {round(totalValue, 2)} worth of share(s).\n")
    return redirect("/")
...
def sell():
    ...
    if not selectedSymbol:
        ...
        return render_template("sell.html", symbolAndShareData=symbolAndShareData), 400
    if not sharesStr:
    ...
    if (selectedSharePrice == 0):
        ...
        return render_template("sell.html", symbolAndShareData=symbolAndShareData), 404
...
def addCredit():
    ...
    if not creditStr:
        flash("Credit amount is required."), 400
        return redirect("/account")

    credit = float(creditStr)
    if credit == 0:
        flash("Credit amount has to be a positive integer."), 400
        return redirect("/account")
